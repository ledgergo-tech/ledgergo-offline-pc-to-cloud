...
function showInvoicePrint(invoice) {
  ...
  document.querySelector("#print-modal .modal-title").textContent = `Invoice Preview: ${invoiceNumber}`;
...
function showAddProduct(prod) {
  ...
  // Auto-generate 10-digit Barcode if new item
  let autoBarcode = "";
  if (!prod) {
    autoBarcode = Math.floor(1000000000 + Math.random() * 9000000000).toString();
  }

  // Tab 1: General Info
  document.getElementById("content-basic").innerHTML = `
    ${prod ? `<input type="hidden" name="id" value="${prod.id}" form="item-modal-form">` : ""}
    <div class="form-grid-2">
      ...
      <div class="form-group">
        ...
        <input name="barcode" value="${prod ? prod.barcode || "" : autoBarcode}" placeholder="Scan / type barcode" />
      </div>
      <div class="form-group">
        <label class="form-group-label">Category</label>
        <div class="input-group">
          <input name="category" id="item-category-input" value="${prod ? prod.category || "" : ""}" placeholder="Search or type new category..." onfocus="showCategoryPicker(true)" />
          <button type="button" class="btn-plus" onclick="showCategoryManager()" title="Manage Categories" style="min-width:38px;">⚙️</button>
        </div>
        <div id="category-suggestions" style="margin-top:8px; display:flex; flex-wrap:wrap; gap:6px; max-height:100px; overflow-y:auto; padding:4px; border:1px solid transparent;">
          ${categories.map(c => `<span class="badge" style="cursor:pointer; background:#f1f5f9; color:var(--text-primary); border:1px solid var(--border); text-transform:none;" onclick="selectCategory('${c.replace(/'/g, "\\'")}')">${c}</span>`).join("")}
          ${categories.length === 0 ? '<span class="text-secondary" style="font-size:11px;">No existing categories. Type to create.</span>' : ''}
        </div>
      </div>
      <div class="form-group">
...
function showCategoryManager() {
  ...
  if (!modal) return;
  modal.style.display = "flex";
  document.getElementById("print-modal-title").textContent = "All Categories Master";
  ...
  `;
  document.getElementById("print-modal-footer").innerHTML = `<button class="btn btn-outline" onclick="closePrintModal()">Close Manager</button>`;
}
...
function recalcInvoiceTotal() {
  ...
  const afterLedger = total + ledgerBalance - paid;
